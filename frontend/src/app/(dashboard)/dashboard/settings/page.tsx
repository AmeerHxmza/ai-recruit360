...
export default function SettingsPage() {
  ...
  useEffect(() => {
    async function loadProfile() {
      ...
      if (user) {
        ...
        // Logo is still loaded from local storage for now
  ...
  const handleSave = async () => {
    ...
    // Optionally update the recruiters table
    const { data: { user } } = await supabase.auth.getUser();
    if (user) {
    ...
    // Force a router refresh to update Server Components (like the Header Layout)
    router.refresh();
        ...
        <Card className="surface-card">
          ...
          <CardContent className="space-y-6">
            <div className="space-y-4">
              ...
              <div className="grid gap-2">
                <Label htmlFor="recruiter-email">Email Account</Label>
                <Input
                  id="recruiter-email"
                  type="email"
                  value={email}
            ...
            <Button variant="accent" className="w-full sm:w-auto" onClick={handleSave} disabled={isSaving}>
